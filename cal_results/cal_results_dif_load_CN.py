...
def plot_data(folder_paths, names, colors, markers, save_plots=False, least_num=False):
    ...
    for attribute in attributes:
        ...
        for folder_path, name in zip(folder_paths, names):
            ...
            data_dict[(folder_path, name)] = (avg_totals, values)
            min_length = min(min_length, len(avg_totals), len(values))

        for folder_path, name, color, marker in zip(folder_paths, names, colors, markers):
            avg_totals, values = data_dict[(folder_path, name)]
            if least_num:
                avg_totals = avg_totals[-min_length:]
        ...
        if save_plots:
            ...
            plt.savefig(f"./images/{attribute}_dif_load_CN.pdf", format='pdf')
            plt.savefig(f"./images/{attribute}_dif_load_CN.png", format='png')
        plt.show()
