...
def load_config(yaml_path):
    ...
    return folder_paths, names, save_plots
...
def plot_data(folder_paths, names, colors, markers,save_plots=False):
    ...
    for attribute in attributes:
        ...
        for folder_path, name, color, marker in zip(folder_paths, names, colors, markers):
            ...
            for file_name in os.listdir(folder_path):
                ...
                if os.path.isfile(file_path) and file_path.endswith('.txt'):
                    ...
                    avg_totals.append(parsed_data['Average Total'])
                    values.append(parsed_data[attribute])

            plt.plot(avg_totals, values, label=name, color=color, marker=marker)
            min_totals, max_totals = min(avg_totals), max(avg_totals)
        plt.xlim(min_totals, max_totals)

        plt.xlabel('Average tasks per second')
        ...
        if save_plots:
            ...
            plt.savefig(f"./images/{attribute}_dif_load.pdf", format='pdf')
            plt.savefig(f"./images/{attribute}_dif_load.png", format='png')
        plt.show()

colors = ['#9467bd', '#7f7f7f', '#17becf', '#bcbd22', '#1f77b4', '#e377c2']
markers = ['o', 's', '^', 'D', '*', 'X']
...
folder_paths, names,save_plots = load_config(args.config)

plot_data(folder_paths, names, colors[:len(folder_paths)], markers[:len(folder_paths)],save_plots)